...
function model = cbInstanceLSML( X, Y, optmParameter)
...
%       model    -  a structure variable composed of the model coefficients
   ...
   %% optimization parameters
    ...
    %lambda2 and lambda3 in paper are swapped
   ...
   %% initialization
    ...
    B = computeCostSensitiveMatrix(Y, 1.2); 
    maxB2 = max(B.^2, [], 'all');
    ...
    while iter <= maxIter
      ...
      %% Loss
       ...
       DiscriminantLoss = norm((LS .* LS).*B, 1);
       LS = Y*C - Y;
       ...
       sparesW    = sum(sum(W~=0));
       sparesC    = sum(sum(C~=0));
       totalloss = DiscriminantLoss + lambda1*CorrelationLoss + ...
            lambda2*sparesW + lambda3*sparesC+lambda4*CorrelationLoss2 +...
            lambda5*InstanceSimilarityLoss;
       ...
       %fprintf('%0.4f  %0.4f  %0.4f  %0.4f  %0.4f  %0.4f\n', DiscriminantLoss, ...
       %    lambda1*CorrelationLoss, lambda3*sparesC, lambda2*sparesW,    ...
       %    lambda4*CorrelationLoss2, lambda5* InstanceSimilarityLoss );
